...
When the Proxmox VE boot menu appears, highlight **Install Proxmox VE (Graphical)** but do **not** press Enter. Press **`e`** instead to open the GRUB boot editor.
...
Pressing `e` opens the GRUB boot entry editor showing the kernel boot parameters.
...
Press **Ctrl + X** to boot with the modified parameters. The installer will load in legacy graphics mode and proceed normally to the EULA screen.
